window.addEventListener("load", function(){
    ...
    $searchInput.addEventListener("input", () => {
        ...
        if ($searchInput.value.lenght != 0) {
            ...
            fetch(`https://multihogar.onrender.com/api/users/search?search=${$searchInput.value}`)
            //http://localhost:3003 dev https://multihogar.onrender.com production
            .then(res => res.json())
            .then(info =>{
                if(info.data.length > 0){
                    for (const user of info.data) {
                        $list.innerHTML += `<li><a href="/dashboard/users/${user.id}"> ${user.name} - ${user.username} - ${user.email} </a></li>`
                    }
                } else {
